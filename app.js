// Amber的活动手�?- 网页�?v4 云端版（Supabase�?// 活动管理 + 多日期范�?+ 签到 + 薪资结算 + 积分兑换
...
async function init() {
  ...
  let adminArr = await sbGet('admin', { name: 'eq.管理�?, limit: 1 });
  if (!adminArr || adminArr.length === 0) {
    await sbPost('admin', { name: '管理�?, phone: '13800000000', password: 'admin123', invitecode: 'naidezhuan2026' });
  }
  ...
  if (!acts || acts.length === 0) {
    const demoActs = [
      ...
      { id: 1002, title: '798咖啡品鉴小聚', location: '798艺术�?, content: '一起�?98，找家咖啡馆坐下来聊最近的生活�?, points: 80, people: 6, enrolled: 3, date: '2026-05-28', time: '14:00-17:00', status: 'active', publisher: 'Amber', creator: true },
  ...
  if (!emptyRewards || emptyRewards.length === 0) {
    const demoRewards = [
      ...
      { id: 2, name: '电影票一�?, points_cost: 300, stock: 5, status: 'active', created_at: new Date().toISOString() },
...
async function login(name, isAdmin, pwd) {
  if (isAdmin) {
    const admins = await sbGet('admin', { name: 'eq.管理�?, limit: 1 });
    const admin = admins[0];
    if (!admin) return '管理员未初始�?;
...
function loginPage() {
  ...
  <div class="login-box">
    ...
    <div class="form-group" id="adminPwdGroup" style="display:none">
      <input class="form-input" id="loginPwd" type="password" placeholder="输入管理员密�?>
    </div>
    <div class="login-btn-group">
      <button class="btn btn-primary" id="loginBtn">进入</button>
    </div>
    <div style="text-align:center;margin-top:12px">
      <span id="adminToggle" style="font-size:13px;color:var(--primary);cursor:pointer">管理员登�?/span>
    </div>
...
function homePage() {
  ...
  let html = `<div class="header"><h1>📒 那得赚一�?/h1><p>看看朋友们最近在玩什�?/p></div>`;
  ...
  if (!acts.length) {
    ...
    acts.forEach(a => {
      ...
      if (a.start_date && a.end_date) {
        dateDisplay = a.start_date + ' �?' + a.end_date;
      }
      html += `<div class="card" onclick="navigate('detail',${a.id})">
        ...
        <div class="card-footer">
          <span>👥 ${a.enrolled || 0}/${a.people || 0}�?/span>
          <span class="tag">进行�?/span>
        </div>
        ${e ? '<div class="btn btn-gray" style="margin:10px 0 0">�?已参�?/div>'
          : '<div class="btn btn-primary" style="margin:10px 0 0" data-enroll="' + a.id + '" onclick="event.stopPropagation();enroll(' + a.id + ')">🎯 我想�?/div>'}
      </div>`;
    });
  }
...
function detailPage() {
  ...
  return `<div style="padding:16px">
    ...
    <div class="card" style="margin:0">
      ...
      <div style="margin:8px 0"><span class="tag">进行�?/span></div>
      <div class="card-meta">📍 ${a.location || ''}</div>
      <div class="card-meta">📅 ${dateDisplay}</div>
      ${timeDisplay ? `<div class="card-meta">�?${timeDisplay}</div>` : ''}
      <div class="card-meta">👥 ${a.enrolled || 0}/${a.people || 0}�?/div>
      <div class="card-meta">🎯 积分 +${a.points || 0}</div>
      ${a.require_checkin ? '<div class="card-meta">📍 需要现场签�?/div>' : ''}
    ...
    ${e ? '<div class="btn btn-gray">�?已参�?/div>' : '<div class="btn btn-primary" onclick="enroll(' + a.id + ')">🎯 我想参与</div>'}
  </div>`;
}
...
function myPage() {
  ...
  let html = `<div class="page-title">📋 我的记录</div>
    <div style="padding:8px 16px 0;font-size:14px;color:var(--text-secondary)">查看所有参与记�?/div>`;
  ...
  html += `<div class="filter-tabs">${['all', 'pending', 'approved', 'checkedin', 'completed', 'paid'].map(f =>
    `<span class="filter-tab ${state.filter === f ? 'active' : ''}" onclick="setMyFilter('${f}')">${
      { all: '全部', pending: '待审�?, approved: '已通过', checkedin: '已签�?, completed: '已完�?, paid: '已发�? }[f]
    }</span>`
  ).join('')}</div>`;

  let filtered = apps;
  if (state.filter === 'pending') filtered = apps.filter(a => String(a.status) === '待审�?);
  else if (state.filter === 'approved') filtered = apps.filter(a => String(a.status) === '已通过');
  else if (state.filter === 'checkedin') filtered = apps.filter(a => String(a.status) === '已签�?);
  else if (state.filter === 'completed') filtered = apps.filter(a => String(a.status) === '已完�?);
  else if (state.filter === 'paid') filtered = apps.filter(a => String(a.status) === '已发�?);
  else filtered = apps;
  ...
  if (!filtered.length) {
    ...
    filtered.forEach(app => {
      ...
      // Check-in button: if activity requires checkin and status is 已通过 or 已签�?      if (task && task.require_checkin && (String(app.status) === '已通过' || String(app.status) === '已签�?)) {
        if (String(app.status) === '已签�?) {
          actionBtns += `<span class="tag" style="background:#e8f5e9;color:#2e7d32">�?已签�?/span>`;
        } else {
          actionBtns += `<button class="btn-small" onclick="doCheckin(${app.id})">📍 签到</button>`;
        }
      }

      // Salary apply button: if status is 已完�?and no salaryconfirmed yet
      if (String(app.status) === '已完�? && !app.salaryconfirmed && String(app.status) !== '待发�? && String(app.status) !== '已发�?) {
      ...
      if (app.confirmed_salary || app.salaryconfirmed) {
        actionBtns += `<button class="btn-small" style="background:var(--green)" onclick="navigate('salary_slip',${app.id})">📄 薪资�?/button>`;
      }
  ...
  if (u && u.role === 'admin') {
    html += `<div style="text-align:center;padding:10px;color:var(--primary);cursor:pointer" onclick="navigate('publish')">�?发布新活�?/div>`;
  }
  return html;
}
...
function pointsPage() {
  ...
  return `<div class="points-header">
      ...
      <div class="points-label">参与活动获得的虚拟积�?🎯</div>
    </div>
    <div class="card" style="margin:0 16px 16px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div style="text-align:center;padding:12px;background:#f8f9fc;border-radius:8px">
          ...
          <div style="font-size:12px;color:var(--text-secondary)">💰 累计薪资(�?</div>
        </div>
        <div style="text-align:center;padding:12px;background:#f8f9fc;border-radius:8px">
  ...
  if (!rewards.length) {
    ...
    rewards.forEach(r => {
      ...
      html += `<div class="card">
        ...
        <div class="card-meta">🎯 需�?${r.points_cost || 0} 积分 · 剩余 ${r.stock || 0} �?/div>
        <div style="margin-top:10px">
          ${outOfStock ? `<span class="tag tag-gray">已兑�?/span>` :
...
function salaryApplyPage() {
  ...
  const appRec = state.apps.find(a => String(a.id) === String(appId));
  ...
  return `<div style="padding:16px">
    ...
    <div class="card" style="margin:0;background:linear-gradient(135deg,#ff6b6b,#ff8e8e);color:white">
      <div style="text-align:center;padding:20px 0">
        <div style="font-size:14px;opacity:0.8">💰 薪资确认�?/div>
...
function profilePage() {
  ...
  return `<div class="page-title">⚙️ 设置</div>
    <div class="card" style="text-align:center">
      ...
      <div class="card-meta">${isAdmin ? '🔐 管理�? : '👤 用户'}</div>
      <div class="card-meta">积分�?{userRec.total_points || u.points || 0}</div>
      <div class="card-meta">累计薪资�?{userRec.total_salary || 0}�?/div>
      <div class="card-meta">完成 ${userRec.completed_count || 0} �?/div>
    </div>
    ...
    <div class="card" style="cursor:pointer" onclick="logout()">
      <span style="color:var(--primary)">🚪 退出登�?/span>
    </div>`;
}
...
function adminPage() {
  ...
  const pending = apps.filter(a => String(a.status) === '待审�?).length;
  const salaryPending = apps.filter(a => String(a.status) === '已完�?).length;
  return `<div class="page-title">🔐 管理后台</div>
    ...
    <div class="card">
      ...
      <div class="overview-grid">
        ...
        <div>�?${pending} 待审</div>
        <div>�?${apps.filter(a => a.status === '已完�?).length} 待结�?/div>
      </div>
    </div>
...
function adminTasks() {
  ...
  if (!acts.length) { html += `<div class="empty"><div class="icon">📭</div><p>暂无活动</p></div>`; }
    ...
    acts.forEach(a => {
      html += `<div class="card">
        <div class="card-title">${a.title}</div>
        <div class="card-meta">👥 ${a.enrolled || 0}/${a.people || 0} · 📅 ${a.start_date && a.end_date ? (a.start_date + ' �?' + a.end_date) : (a.date || '未设�?)}</div>
        ${a.time_slots ? `<div class="card-meta">�?${(function(){try{return JSON.parse(a.time_slots).join(' / ');}catch{return a.time_slots;}})()}</div>` : ''}
        <div class="card-footer">
          <span class="tag ${String(a.status) === 'active' ? '' : 'tag-gray'}">${String(a.status) === 'active' ? '进行�? : '已结�?}</span>
          <span>
...
function adminApps() {
  const apps = state.apps;
  const acts = state.activities;
  const pendingCount = apps.filter(a => String(a.status) === '待审�?).length;
  const salaryCount = apps.filter(a => String(a.status) === '已完�?).length;
    ...
    <div class="filter-tabs" style="padding:8px 16px">
      ${['all', 'pending', 'salary', 'paid'].map(f =>
        `<span class="filter-tab ${state.filter === f ? 'active' : ''}" onclick="setAppFilter('${f}')">${
          { all: '全部', pending: `待审�?${pendingCount})`, salary: `待结�?${salaryCount})`, paid: '已发�? }[f]
        }</span>`
      ).join('')}
    </div>`;
  ...
  let filtered = apps;
  if (state.filter === 'pending') filtered = apps.filter(a => String(a.status) === '待审�?);
  else if (state.filter === 'salary') filtered = apps.filter(a => String(a.status) === '已签�? || String(a.status) === '已完�?);
  else if (state.filter === 'paid') filtered = apps.filter(a => String(a.status) === '已发�?);
  ...
  if (!filtered.length) {
    ...
    filtered.forEach(a => {
      ...
      html += `<div class="card">
        ...
        <div class="card-meta">💰 约定薪资: ${a.taskpay || 0}�?· 状�? ${a.status || ''}</div>
        ...
        <div class="card-footer">
          ...
          (String(a.status) === '已签�? || String(a.status) === '已完�?) ?
            `<span>
              ${String(a.status) === '已签�? ? `<button class="btn-small" onclick="completeActivity(${a.id})">�?点完�?/button>` : ''}
          ...
          (String(a.status) === '已发�?) ?
            `<span>
              <span class="tag tag-gray">已发�?¥${a.confirmed_salary || 0}</span>
...
function adminUsers() {
  ...
  if (!users.length) { html += `<div class="empty"><div class="icon">👤</div><p>暂无用户</p></div>`; }
    ...
    users.forEach(u => {
      const completedCount = apps.filter(a => (a.userid === u.name || a.userid === u.id) && (a.status === '已完�? || a.status === '已发�?)).length;
      ...
      html += `<div class="card">
        ...
        <div class="card-meta">
          ...
          �?完成: ${u.completed_count || completedCount} �?        </div>
  ...
  if (!rewards.length) { html += `<div class="empty"><div class="icon">🎁</div><p>暂无奖品</p></div>`; }
    ...
    rewards.forEach(r => {
      html += `<div class="card">
        ...
        <div class="card-footer">
          <span class="tag ${String(r.status) === 'active' ? '' : 'tag-gray'}">${String(r.status) === 'active' ? '上架�? : '已下�?}</span>
          <span>
...
function adminPublish() {
  return `<div class="page-title">📝 发布新活�?/div>
    ...
    <div class="form-group">
      ...
      <div style="font-size:12px;color:var(--text-secondary);margin-top:4px">多个时段用英文逗号分隔，如�?9:00-11:00, 14:00-17:00</div>
    </div>
...
async function doPublish() {
  ...
  if (!title || !location) {
    document.getElementById('pubError').textContent = '请填写必填项：标题、地�?;
    return;
  }
...
async function rejectApp(id) {
  ...
  await sbPut('apps', { id: 'eq.' + id }, { status: '已拒�?, rejectreason: reason });
  render();
}

async function completeActivity(id) {
  if (!confirm('确认标记为完成？')) return;
  const updates = { status: '已完�?, completed_at: new Date().toISOString() };
...
function showSalaryConfirmForm(appId) {
  ...
  app.innerHTML = `<div style="padding:16px">
    ...
    <div class="form-group">
      <label class="form-label">📋 凭证（截�?单号�?/label>
      <input class="form-input" id="salaryScreenshot" placeholder="输入截图描述或单�?>
    </div>
...
async function redeemReward(rewardId) {
  ...
  if (!confirm(`确认�?${reward.points_cost} 积分兑换 "${reward.name}"？`)) return;
...
function copySalarySlip(appId) {
  const appRec = state.apps.find(a => String(a.id) === String(appId));
  if (!appRec) return;
  const text = `【薪资确认条�?活动�?{appRec.tasktitle || ''}
...
确认时间�?{appRec.confirmed_at ? appRec.confirmed_at.slice(0, 16).replace('T', ' ') : ''}`;
  if (navigator.clipboard) {
    navigator.clipboard.writeText(text).then(() => alert('已复制到剪贴�?));
  } else {
    prompt('复制以下信息�?, text);
  }
}
...
function bindEvents() {
  ...
  if (loginBtn) {
    ...
    document.getElementById('loginName')?.addEventListener('input', () => {
      ...
      if (adminPwdGroup) {
        // Show admin pwd for both "管理�? keyword and the phone number
        adminPwdGroup.style.display = (v === '管理�? || v === '13800000000') ? 'block' : 'none';
      }
    });
